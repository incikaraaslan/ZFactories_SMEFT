...
MAbool user::Execute(SampleFormat& sample, const EventFormat& event)
{
  ...
  {
  {
    ...
    {
      Manager()->FillHisto("4_PT", _P_e_p_I1I_PTorderingfinalstate_REG_[ind[0]]->pt());
    }
  }
  }
  ...
  {
  {
    ...
    for (ind[0]=0;ind[0]<_P_e_p_I1I_PTorderingfinalstate_REG_.size();ind[0]++)
    {
      Manager()->FillHisto("5_ETA", _P_e_p_I1I_PTorderingfinalstate_REG_[ind[0]]->eta());
    }
  }
  }
  ...
  {
    MAuint32 a[1];
    for (a[0]=0;a[0]<_P_e_p_I1I_PTorderingfinalstate_REG_.size();a[0]++)
    {
    MAuint32 b[1];
    for (b[0]=0;b[0]<_P_ta_m_I1I_PTorderingfinalstate_REG_.size();b[0]++)
    {
      Manager()->FillHisto("9_DELTAR", _P_e_p_I1I_PTorderingfinalstate_REG_[a[0]]->dr(_P_ta_m_I1I_PTorderingfinalstate_REG_[b[0]]));
    }
    }
  }
  }

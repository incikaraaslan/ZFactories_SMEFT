...
MAbool user::Execute(SampleFormat& sample, const EventFormat& event)
{
  ...
  {
  {
    ...
    for (ind[0]=0;ind[0]<_P_e_p_I1I_PTorderingfinalstate_REG_.size();ind[0]++)
    {
    for (ind[1]=0;ind[1]<_P_e_m_I1I_PTorderingfinalstate_REG_.size();ind[1]++)
    {
    ParticleBaseFormat q;
    q+=_P_e_p_I1I_PTorderingfinalstate_REG_[ind[0]]->momentum();
    q+=_P_e_m_I1I_PTorderingfinalstate_REG_[ind[1]]->momentum();
  ...
  {
  {
    ...
    {
      Manager()->FillHisto("9_DELTAR", _P_e_m_I1I_PTorderingfinalstate_REG_[a[0]]->dr(_P_e_p_I1I_PTorderingfinalstate_REG_[b[0]]));
    }
    }
  }
  }

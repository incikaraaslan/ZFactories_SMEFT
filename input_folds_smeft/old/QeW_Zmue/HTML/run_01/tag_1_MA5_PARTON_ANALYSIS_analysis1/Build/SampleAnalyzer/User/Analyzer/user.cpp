...
MAbool user::Execute(SampleFormat& sample, const EventFormat& event)
{
  ...
  {
  {
    ...
    {
      Manager()->FillHisto("6_PT", _P_mu_m_I1I_PTorderingfinalstate_REG_[ind[0]]->pt());
    }
  }
  }
  ...
  {
  {
    ...
    for (ind[0]=0;ind[0]<_P_mu_m_I1I_PTorderingfinalstate_REG_.size();ind[0]++)
    {
      Manager()->FillHisto("7_ETA", _P_mu_m_I1I_PTorderingfinalstate_REG_[ind[0]]->eta());
    }
  }
  }
  ...
  {
  {
    ...
    for (ind[0]=0;ind[0]<_P_e_p_I1I_PTorderingfinalstate_REG_.size();ind[0]++)
    {
    for (ind[1]=0;ind[1]<_P_mu_m_I1I_PTorderingfinalstate_REG_.size();ind[1]++)
    {
    ParticleBaseFormat q;
    q+=_P_e_p_I1I_PTorderingfinalstate_REG_[ind[0]]->momentum();
    q+=_P_mu_m_I1I_PTorderingfinalstate_REG_[ind[1]]->momentum();
  ...
  {
  {
    ...
    {
    MAuint32 b[1];
    for (b[0]=0;b[0]<_P_mu_m_I1I_PTorderingfinalstate_REG_.size();b[0]++)
    {
      Manager()->FillHisto("9_DELTAR", _P_e_p_I1I_PTorderingfinalstate_REG_[a[0]]->dr(_P_mu_m_I1I_PTorderingfinalstate_REG_[b[0]]));
    }
    }
  }
  }

...
MAbool user::Execute(SampleFormat& sample, const EventFormat& event)
{
  ...
  // Clearing particle containers
  {
      ...
      _P_e_p_I1I_PTorderingfinalstate_REG_.clear();
      _P_ta_mPTorderingfinalstate_REG_.clear();
      _P_e_pPTorderingfinalstate_REG_.clear();
  }
  ...
  // Filling particle containers
  {
    for (MAuint32 i=0;i<event.mc()->particles().size();i++)
    {
      ...
      if (isP__e_pPTorderingfinalstate((&(event.mc()->particles()[i])))) _P_e_pPTorderingfinalstate_REG_.push_back(&(event.mc()->particles()[i]));
    }
  }
  ...
  // Sorting particle collection according to PTordering
  // for getting 1th particle
  _P_e_p_I1I_PTorderingfinalstate_REG_=SORTER->rankFilter(_P_e_pPTorderingfinalstate_REG_,1,PTordering);
  ...
  {
    MAuint32 ind[1];
    for (ind[0]=0;ind[0]<_P_e_p_I1I_PTorderingfinalstate_REG_.size();ind[0]++)
    {
      Manager()->FillHisto("6_PT", _P_e_p_I1I_PTorderingfinalstate_REG_[ind[0]]->pt());
    }
  }
  }
  ...
  {
  {
    ...
    for (ind[0]=0;ind[0]<_P_e_p_I1I_PTorderingfinalstate_REG_.size();ind[0]++)
    {
      Manager()->FillHisto("7_ETA", _P_e_p_I1I_PTorderingfinalstate_REG_[ind[0]]->eta());
    }
  }
  }
  ...
  {
  {
    ...
    q+=_P_e_p_I1I_PTorderingfinalstate_REG_[ind[0]]->momentum();
  ...
  {
  {
    ...
    {
    MAuint32 b[1];
    for (b[0]=0;b[0]<_P_e_p_I1I_PTorderingfinalstate_REG_.size();b[0]++)
    {
      Manager()->FillHisto("9_DELTAR", _P_ta_m_I1I_PTorderingfinalstate_REG_[a[0]]->dr(_P_e_p_I1I_PTorderingfinalstate_REG_[b[0]]));
    }
    }
  }
  }

...
MAbool user::Execute(SampleFormat& sample, const EventFormat& event)
{
  ...
  {
  {
    ...
    q+=_P_mu_m_I1I_PTorderingfinalstate_REG_[ind[0]]->momentum();
  ...
  {
    MAuint32 a[1];
    for (a[0]=0;a[0]<_P_mu_m_I1I_PTorderingfinalstate_REG_.size();a[0]++)
    {
    MAuint32 b[1];
    for (b[0]=0;b[0]<_P_ta_p_I1I_PTorderingfinalstate_REG_.size();b[0]++)
    {
      Manager()->FillHisto("9_DELTAR", _P_mu_m_I1I_PTorderingfinalstate_REG_[a[0]]->dr(_P_ta_p_I1I_PTorderingfinalstate_REG_[b[0]]));
    }
    }
  }
  }

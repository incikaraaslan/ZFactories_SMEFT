...
C       LP=SIGN(1,LPP(2))
        H2=PDG2PDF(LPP(2),25, 2,XBK(2),DSQRT(Q2FACT(2)))
      ENDIF
      PD(0) = 0D0
      IPROC = 0
      IPROC=IPROC+1  ! z h > mu- ta+
      PD(IPROC)=Z1*H2
...
C     
      ...
      DOUBLE PRECISION H2(VECSIZE_MEMMAX)
      DOUBLE PRECISION XPQ(-7:7),PD(0:MAXPROC)
...
C           LP=SIGN(1,LPP(1))
     ...
     $       ,DSQRT(ALL_Q2FACT(1,IVEC)))
          ...
          IF (ABS(LPP(2)) .GE. 1) THEN
C           LP=SIGN(1,LPP(2))
            H2(IVEC)=PDG2PDF(LPP(2),25, 2,ALL_XBK(2,IVEC)
     $       ,DSQRT(ALL_Q2FACT(2,IVEC)))
      ...
      DO IVEC=1, VECSIZE_USED
        ALL_PD(IPROC,IVEC)=Z1(IVEC)*H2(IVEC)

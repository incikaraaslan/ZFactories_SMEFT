...
C       LP=SIGN(1,LPP(1))
        Z1=PDG2PDF(LPP(1),23, 1,XBK(1),DSQRT(Q2FACT(1)))
      ENDIF
      IF (ABS(LPP(2)) .GE. 1) THEN
C       LP=SIGN(1,LPP(2))
        WM2=PDG2PDF(LPP(2),-24, 2,XBK(2),DSQRT(Q2FACT(2)))
      ENDIF
      PD(0) = 0D0
      IPROC = 0
      IPROC=IPROC+1  ! z w- > e- ve~
      PD(IPROC)=Z1*WM2
...
C     
      INTEGER I,ITYPE,LP,IPROC
      DOUBLE PRECISION QSCALE
      DOUBLE PRECISION Z1(VECSIZE_MEMMAX)
      DOUBLE PRECISION WM2(VECSIZE_MEMMAX)
      DOUBLE PRECISION XPQ(-7:7),PD(0:MAXPROC)
...
C     DATA
C     
      DATA Z1/VECSIZE_MEMMAX*1D0/
      DATA WM2/VECSIZE_MEMMAX*1D0/
C     ----------
C     BEGIN CODE
C     ----------
      SELECTED_HEL(:) = 0
     ...
     &    NB_WARP_USED, WARP_SIZE, VECSIZE_USED
      ...
      DO CURR_WARP=1, NB_WARP_USED
        ...
        DO IWARP=1, WARP_SIZE
          ...
          IF (ABS(LPP(1)) .GE. 1) THEN
C           LP=SIGN(1,LPP(1))
            Z1(IVEC)=PDG2PDF(LPP(1),23, 1,ALL_XBK(1,IVEC)
     $       ,DSQRT(ALL_Q2FACT(1,IVEC)))
          ENDIF
          IF (ABS(LPP(2)) .GE. 1) THEN
C           LP=SIGN(1,LPP(2))
            WM2(IVEC)=PDG2PDF(LPP(2),-24, 2,ALL_XBK(2,IVEC)
     $       ,DSQRT(ALL_Q2FACT(2,IVEC)))
      ...
      DO IVEC=1, VECSIZE_USED
        ALL_PD(IPROC,IVEC)=Z1(IVEC)*WM2(IVEC)

...
MAbool user::Execute(SampleFormat& sample, const EventFormat& event)
{
  ...
  {
      _P_u_I1I_PTorderingfinalstate_REG_.clear();
      _P_u_t_I1I_PTorderingfinalstate_REG_.clear();
      _P_uPTorderingfinalstate_REG_.clear();
      _P_u_tPTorderingfinalstate_REG_.clear();
  }
  ...
  // Filling particle containers
  {
    ...
    {
      if (isP__uPTorderingfinalstate((&(event.mc()->particles()[i])))) _P_uPTorderingfinalstate_REG_.push_back(&(event.mc()->particles()[i]));
      if (isP__u_tPTorderingfinalstate((&(event.mc()->particles()[i])))) _P_u_tPTorderingfinalstate_REG_.push_back(&(event.mc()->particles()[i]));
    }
  }
  ...
  _P_u_I1I_PTorderingfinalstate_REG_=SORTER->rankFilter(_P_uPTorderingfinalstate_REG_,1,PTordering);

  // Sorting particle collection according to PTordering
  // for getting 1th particle
  _P_u_t_I1I_PTorderingfinalstate_REG_=SORTER->rankFilter(_P_u_tPTorderingfinalstate_REG_,1,PTordering);
  ...
  {
    MAuint32 ind[1];
    for (ind[0]=0;ind[0]<_P_u_I1I_PTorderingfinalstate_REG_.size();ind[0]++)
    {
      Manager()->FillHisto("4_PT", _P_u_I1I_PTorderingfinalstate_REG_[ind[0]]->pt());
    }
  }
  }
  ...
  {
  {
    ...
    for (ind[0]=0;ind[0]<_P_u_I1I_PTorderingfinalstate_REG_.size();ind[0]++)
    {
      Manager()->FillHisto("5_ETA", _P_u_I1I_PTorderingfinalstate_REG_[ind[0]]->eta());
    }
  }
  }
  ...
  {
    MAuint32 ind[1];
    for (ind[0]=0;ind[0]<_P_u_t_I1I_PTorderingfinalstate_REG_.size();ind[0]++)
    {
      Manager()->FillHisto("6_PT", _P_u_t_I1I_PTorderingfinalstate_REG_[ind[0]]->pt());
    }
  }
  }
  ...
  {
  {
    ...
    for (ind[0]=0;ind[0]<_P_u_t_I1I_PTorderingfinalstate_REG_.size();ind[0]++)
    {
      Manager()->FillHisto("7_ETA", _P_u_t_I1I_PTorderingfinalstate_REG_[ind[0]]->eta());
    }
  }
  }
  ...
  {
  {
    ...
    for (ind[0]=0;ind[0]<_P_u_I1I_PTorderingfinalstate_REG_.size();ind[0]++)
    {
    for (ind[1]=0;ind[1]<_P_u_t_I1I_PTorderingfinalstate_REG_.size();ind[1]++)
    {
    ParticleBaseFormat q;
    q+=_P_u_I1I_PTorderingfinalstate_REG_[ind[0]]->momentum();
    q+=_P_u_t_I1I_PTorderingfinalstate_REG_[ind[1]]->momentum();
  ...
  {
    MAuint32 a[1];
    for (a[0]=0;a[0]<_P_u_I1I_PTorderingfinalstate_REG_.size();a[0]++)
    {
    MAuint32 b[1];
    for (b[0]=0;b[0]<_P_u_t_I1I_PTorderingfinalstate_REG_.size();b[0]++)
    {
      Manager()->FillHisto("9_DELTAR", _P_u_I1I_PTorderingfinalstate_REG_[a[0]]->dr(_P_u_t_I1I_PTorderingfinalstate_REG_[b[0]]));
    }
    }
  }
  }

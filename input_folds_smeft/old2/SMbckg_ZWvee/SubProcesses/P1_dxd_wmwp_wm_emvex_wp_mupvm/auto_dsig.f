...
C     
C     Process: d~ d > w- w+ WEIGHTED<=4 @1
C     *   Decay: w- > e- ve~ WEIGHTED<=2
C     *   Decay: w+ > mu+ vm WEIGHTED<=2
C     Process: s~ s > w- w+ WEIGHTED<=4 @1
C     *   Decay: w- > e- ve~ WEIGHTED<=2
C     *   Decay: w+ > mu+ vm WEIGHTED<=2
C     
...
C       LP=SIGN(1,LPP(2))
      ...
      PD(0) = 0D0
      IPROC = 0
      IPROC=IPROC+1  ! d~ d > e- ve~ mu+ vm
      PD(IPROC)=DX1*D2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! s~ s > e- ve~ mu+ vm
      PD(IPROC)=SX1*S2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IF (IMODE.EQ.4)THEN
        DSIG = PD(0)
...
C     
      INTEGER I,ITYPE,LP,IPROC
      DOUBLE PRECISION QSCALE
      DOUBLE PRECISION SX1(VECSIZE_MEMMAX),DX1(VECSIZE_MEMMAX)
      DOUBLE PRECISION D2(VECSIZE_MEMMAX),S2(VECSIZE_MEMMAX)
      DOUBLE PRECISION XPQ(-7:7),PD(0:MAXPROC)
...
C     DATA
C     
      DATA SX1,DX1/VECSIZE_MEMMAX*1D0,VECSIZE_MEMMAX*1D0/
      DATA D2,S2/VECSIZE_MEMMAX*1D0,VECSIZE_MEMMAX*1D0/
C     ----------
C     BEGIN CODE
C     ----------
      SELECTED_HEL(:) = 0
     ...
     &    NB_WARP_USED, WARP_SIZE, VECSIZE_USED
      ...
      DO CURR_WARP=1, NB_WARP_USED
        ...
        DO IWARP=1, WARP_SIZE
          ...
          IF (ABS(LPP(1)) .GE. 1) THEN
C           LP=SIGN(1,LPP(1))
            SX1(IVEC)=PDG2PDF(LPP(1),-3, 1,ALL_XBK(1,IVEC)
     $       ,DSQRT(ALL_Q2FACT(1,IVEC)))
            DX1(IVEC)=PDG2PDF(LPP(1),-1, 1,ALL_XBK(1,IVEC)
     $       ,DSQRT(ALL_Q2FACT(1,IVEC)))
          ENDIF
          IF (ABS(LPP(2)) .GE. 1) THEN
C           LP=SIGN(1,LPP(2))
            D2(IVEC)=PDG2PDF(LPP(2),1, 2,ALL_XBK(2,IVEC)
     $       ,DSQRT(ALL_Q2FACT(2,IVEC)))
            S2(IVEC)=PDG2PDF(LPP(2),3, 2,ALL_XBK(2,IVEC)
     $       ,DSQRT(ALL_Q2FACT(2,IVEC)))
          ENDIF
        ENDDO  ! IWARP LOOP
      ENDDO  ! CURRWARP LOOP
      ...
      DO IVEC=1, VECSIZE_USED
        ALL_PD(IPROC,IVEC)=DX1(IVEC)*D2(IVEC)
        ALL_PD(0,IVEC)=ALL_PD(0,IVEC)+DABS(ALL_PD(IPROC,IVEC))

      ENDDO
      IPROC=IPROC+1  ! s~ s > e- ve~ mu+ vm
      DO IVEC=1, VECSIZE_USED
        ALL_PD(IPROC,IVEC)=SX1(IVEC)*S2(IVEC)
        ALL_PD(0,IVEC)=ALL_PD(0,IVEC)+DABS(ALL_PD(IPROC,IVEC))

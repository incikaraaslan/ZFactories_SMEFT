...
C     
      INTEGER I,ITYPE,LP,IPROC
      DOUBLE PRECISION QSCALE
      DOUBLE PRECISION D1(VECSIZE_MEMMAX),S1(VECSIZE_MEMMAX)
      DOUBLE PRECISION SX2(VECSIZE_MEMMAX),DX2(VECSIZE_MEMMAX)
      DOUBLE PRECISION XPQ(-7:7),PD(0:MAXPROC)
...
C     DATA
C     
      DATA D1,S1/VECSIZE_MEMMAX*1D0,VECSIZE_MEMMAX*1D0/
      DATA SX2,DX2/VECSIZE_MEMMAX*1D0,VECSIZE_MEMMAX*1D0/
C     ----------
C     BEGIN CODE
C     ----------
      SELECTED_HEL(:) = 0
     ...
     &    NB_WARP_USED, WARP_SIZE, VECSIZE_USED
      ...
      DO CURR_WARP=1, NB_WARP_USED
        ...
        DO IWARP=1, WARP_SIZE
          ...
          IF (ABS(LPP(1)) .GE. 1) THEN
C           LP=SIGN(1,LPP(1))
            D1(IVEC)=PDG2PDF(LPP(1),1, 1,ALL_XBK(1,IVEC)
     $       ,DSQRT(ALL_Q2FACT(1,IVEC)))
            S1(IVEC)=PDG2PDF(LPP(1),3, 1,ALL_XBK(1,IVEC)
     $       ,DSQRT(ALL_Q2FACT(1,IVEC)))
          ENDIF
          IF (ABS(LPP(2)) .GE. 1) THEN
C           LP=SIGN(1,LPP(2))
            SX2(IVEC)=PDG2PDF(LPP(2),-3, 2,ALL_XBK(2,IVEC)
     $       ,DSQRT(ALL_Q2FACT(2,IVEC)))
            DX2(IVEC)=PDG2PDF(LPP(2),-1, 2,ALL_XBK(2,IVEC)
     $       ,DSQRT(ALL_Q2FACT(2,IVEC)))
          ENDIF
        ENDDO  ! IWARP LOOP
      ENDDO  ! CURRWARP LOOP
      ...
      DO IVEC=1, VECSIZE_USED
        ALL_PD(IPROC,IVEC)=D1(IVEC)*DX2(IVEC)
        ALL_PD(0,IVEC)=ALL_PD(0,IVEC)+DABS(ALL_PD(IPROC,IVEC))

      ENDDO
      IPROC=IPROC+1  ! s s~ > e- ve~ mu+ vm
      DO IVEC=1, VECSIZE_USED
        ALL_PD(IPROC,IVEC)=S1(IVEC)*SX2(IVEC)
        ALL_PD(0,IVEC)=ALL_PD(0,IVEC)+DABS(ALL_PD(IPROC,IVEC))

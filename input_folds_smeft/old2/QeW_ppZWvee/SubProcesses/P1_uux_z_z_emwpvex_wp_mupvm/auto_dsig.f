...
C     
      INTEGER I,ITYPE,LP,IPROC
      DOUBLE PRECISION QSCALE
      DOUBLE PRECISION U1(VECSIZE_MEMMAX),C1(VECSIZE_MEMMAX)
...
C     DATA
C     
      DATA U1,C1/VECSIZE_MEMMAX*1D0,VECSIZE_MEMMAX*1D0/
...
C           LP=SIGN(1,LPP(1))
     ...
     $       ,DSQRT(ALL_Q2FACT(1,IVEC)))
            C1(IVEC)=PDG2PDF(LPP(1),4, 1,ALL_XBK(1,IVEC)
     $       ,DSQRT(ALL_Q2FACT(1,IVEC)))
          ENDIF
          IF (ABS(LPP(2)) .GE. 1) THEN
C           LP=SIGN(1,LPP(2))
            CX2(IVEC)=PDG2PDF(LPP(2),-4, 2,ALL_XBK(2,IVEC)
     ...
     $       ,DSQRT(ALL_Q2FACT(2,IVEC)))
      ...
      IPROC=IPROC+1  ! c c~ > e- mu+ vm ve~
      DO IVEC=1, VECSIZE_USED
        ALL_PD(IPROC,IVEC)=C1(IVEC)*CX2(IVEC)
        ALL_PD(0,IVEC)=ALL_PD(0,IVEC)+DABS(ALL_PD(IPROC,IVEC))

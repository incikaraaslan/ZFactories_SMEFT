...
C       LP=SIGN(1,LPP(2))
      ...
      PD(0) = 0D0
      IPROC = 0
      IPROC=IPROC+1  ! u~ u > e- mu+ vm ve~
      PD(IPROC)=UX1*U2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! c~ c > e- mu+ vm ve~
      PD(IPROC)=CX1*C2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IF (IMODE.EQ.4)THEN
        DSIG = PD(0)
...
C     
      INTEGER I,ITYPE,LP,IPROC
      DOUBLE PRECISION QSCALE
      DOUBLE PRECISION CX1(VECSIZE_MEMMAX),UX1(VECSIZE_MEMMAX)
      DOUBLE PRECISION U2(VECSIZE_MEMMAX),C2(VECSIZE_MEMMAX)
      DOUBLE PRECISION XPQ(-7:7),PD(0:MAXPROC)
...
C     DATA
C     
      DATA CX1,UX1/VECSIZE_MEMMAX*1D0,VECSIZE_MEMMAX*1D0/
      DATA U2,C2/VECSIZE_MEMMAX*1D0,VECSIZE_MEMMAX*1D0/
C     ----------
C     BEGIN CODE
C     ----------
      SELECTED_HEL(:) = 0
     ...
     &    NB_WARP_USED, WARP_SIZE, VECSIZE_USED
      ...
      DO CURR_WARP=1, NB_WARP_USED
        ...
        DO IWARP=1, WARP_SIZE
          ...
          IF (ABS(LPP(1)) .GE. 1) THEN
C           LP=SIGN(1,LPP(1))
            CX1(IVEC)=PDG2PDF(LPP(1),-4, 1,ALL_XBK(1,IVEC)
     $       ,DSQRT(ALL_Q2FACT(1,IVEC)))
            UX1(IVEC)=PDG2PDF(LPP(1),-2, 1,ALL_XBK(1,IVEC)
     $       ,DSQRT(ALL_Q2FACT(1,IVEC)))
          ENDIF
          IF (ABS(LPP(2)) .GE. 1) THEN
C           LP=SIGN(1,LPP(2))
            U2(IVEC)=PDG2PDF(LPP(2),2, 2,ALL_XBK(2,IVEC)
     $       ,DSQRT(ALL_Q2FACT(2,IVEC)))
            C2(IVEC)=PDG2PDF(LPP(2),4, 2,ALL_XBK(2,IVEC)
     $       ,DSQRT(ALL_Q2FACT(2,IVEC)))
          ENDIF
        ENDDO  ! IWARP LOOP
      ENDDO  ! CURRWARP LOOP
      ...
      DO IVEC=1, VECSIZE_USED
        ALL_PD(IPROC,IVEC)=UX1(IVEC)*U2(IVEC)
        ALL_PD(0,IVEC)=ALL_PD(0,IVEC)+DABS(ALL_PD(IPROC,IVEC))

      ENDDO
      IPROC=IPROC+1  ! c~ c > e- mu+ vm ve~
      DO IVEC=1, VECSIZE_USED
        ALL_PD(IPROC,IVEC)=CX1(IVEC)*C2(IVEC)
        ALL_PD(0,IVEC)=ALL_PD(0,IVEC)+DABS(ALL_PD(IPROC,IVEC))
